...
public class Date {
    ...
    /**
     * Metoda pt colectare utilizatori
     */
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password);
            ...
            while (r.next()) {  // adaugare in map
                ...
                if(r.getString(8)!=null)
                    suspendat=true;
                String dataDisponibila=r.getString(9);
    ...
    /**
     * Preluare imprumuturi intarziate
     */
    ...
    /**
     * Preluare ultimele intarzieri cele mai vechi
     */
    ...
    /**
     * Metaoda genereaza continutul istoricului
     * @param a
     * @return
     */
    ...
    public List<Integer> afisareTop(String categorie)
    {
        ...
        if(categorie.equals("Toate"))  // daca este egala cu toate pur si simplu luam din toate
        {
            ...
            List<Integer> carti2=new ArrayList<>();
            for(int i=0;i<carti.size()-1;i++)
                for(int j=i+1;j<carti.size();j++)
                {
                    if(listaCarti.get(carti.get(i)).getNumarImprumuturi()<listaCarti.get(carti.get(j)).getNumarImprumuturi())
                    {
                        int z=carti.get(i);
                        carti.set(i,carti.get(j));
                        carti.set(j,z);
                    }
                }
            int o=10;
